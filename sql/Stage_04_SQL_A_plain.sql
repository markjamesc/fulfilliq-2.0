-- =============================================================================
-- Stage_04_SQL_A_plain.sql — MySQL 8.0; run in one dedicated connection.
-- =============================================================================
-- Spec version : fulfilliq-2.0-stage3-candidate-v0.2.1
-- Sources      : Stage 3 Measurement Design v0.2.1 + Stage 4 shared packet;
--                logic unwrapped from Stage_04_SQL_A.sql procedure body.
-- Engine       : MySQL 8 ordinary session SQL + ordinary a4_* scratch tables
--                (CREATE TEMPORARY TABLES not required).
--                Does NOT use stored procedures, DELIMITER, or CREATE ROUTINE.
-- Privileges   : SELECT on frozen raw_*; CREATE/DROP TABLES;
--                CREATE/DROP TABLE on fulfilliq (for lasting a4_judged_seller).
-- Stop on the first error; do not use mysql --force.
-- No source-table writes. Intermediates are ordinary a4_* scratch tables
-- (dropped at start); final output is lasting fulfilliq.a4_judged_seller.
--
-- Independence (hard):
--   * SQL A must NOT read SQL B, B_orders / B_items / B_sellers, B
--     intermediates, B judgments, R(B), V1 decisions, or recon feedback.
--   * Inputs: frozen fulfilliq.raw_orders, raw_order_items, raw_sellers
--     plus locked Stage 3 / Stage 4 config only.
--   * Do NOT invent featured / catalog / plan-enrollment columns (out of
--     business scope per Stage 3). Do not pad to capacity.
--   * Membership-first, then rank only qualifiers under S.
--
-- Capacity (this run): C=20, simulated O=0 / R=0, S=20
--   (full-capacity simulation; occupancy treated as 0; not authoritative
--   for live enrollment). simulation_S_equals_C_flag = 1.
--
-- Halt pattern: PREPARE/EXECUTE unknown-column assert via @a4_halt_cond
-- / @a4_assert_sql (plain SQL; no SIGNAL handlers). Failing SELECT is
-- prepared only when @a4_halt_cond=1 so the assert cannot false-fire.
-- Client must abort on error.
-- =============================================================================

USE fulfilliq;

-- Documented session vars for this run (not NULL).
...
SET @a4_documented_hold_reason = NULL;

-- Capacity is fixed in judged output: C=20, simulated O=0/R=0, S=20.
...
SET @a4_v_bad = 0;
SET @a4_v_columns = 0;
SET @a4_extracted_at = NULL;


DROP TABLE IF EXISTS a4_evidence, a4_audit, a4_base,
...
-- Halt if unverified/missing snapshot manifest fields.
...
SET @a4_assert_sql = IF(COALESCE(@a4_halt_cond,0) = 1,
  'SELECT `SQL_A_halted:_verified_snapshot_source_boundary_required` FROM (SELECT 1) AS _a4_halt',
  'SELECT 0 INTO @a4_assert');
PREPARE a4_halt_stmt FROM @a4_assert_sql;
EXECUTE a4_halt_stmt;
DEALLOCATE PREPARE a4_halt_stmt;
SET @a4_halt_cond = (VERSION() NOT LIKE '8.0.%');
SET @a4_assert_sql = IF(COALESCE(@a4_halt_cond,0) = 1,
  'SELECT `SQL_A_requires_MySQL_8_0` FROM (SELECT 1) AS _a4_halt',
  'SELECT 0 INTO @a4_assert');
PREPARE a4_halt_stmt FROM @a4_assert_sql;
EXECUTE a4_halt_stmt;
DEALLOCATE PREPARE a4_halt_stmt;
...
SELECT COUNT(*) INTO @a4_v_columns FROM information_schema.columns
...
SET @a4_halt_cond = (@a4_v_columns <> 11);
SET @a4_assert_sql = IF(COALESCE(@a4_halt_cond,0) = 1,
  'SELECT `SQL_A_halted:_missing_required_source_mapping` FROM (SELECT 1) AS _a4_halt',
  'SELECT 0 INTO @a4_assert');
PREPARE a4_halt_stmt FROM @a4_assert_sql;
EXECUTE a4_halt_stmt;
DEALLOCATE PREPARE a4_halt_stmt;
SELECT COUNT(*), COUNT(DISTINCT datetime_precision)
  INTO @a4_v_columns, @a4_v_bad FROM information_schema.columns
...
-- Halt if DATETIME precision/type mapping is not uniform/verified.
SET @a4_halt_cond = (@a4_v_columns <> 5 OR @a4_v_bad <> 1);
SET @a4_assert_sql = IF(COALESCE(@a4_halt_cond,0) = 1,
  'SELECT `SQL_A_halted:_temporal_type_precision_mapping_required` FROM (SELECT 1) AS _a4_halt',
  'SELECT 0 INTO @a4_assert');
PREPARE a4_halt_stmt FROM @a4_assert_sql;
EXECUTE a4_halt_stmt;
DEALLOCATE PREPARE a4_halt_stmt;
...
WHERE table_schema='fulfilliq' AND data_type IN ('char','varchar') AND (
  ...
  (table_name='raw_sellers' AND column_name='seller_id'));
SET @a4_halt_cond = (@a4_v_columns <> 5);
SET @a4_assert_sql = IF(COALESCE(@a4_halt_cond,0) = 1,
  'SELECT `SQL_A_halted:_verify_textual_identity_status_mappings` FROM (SELECT 1) AS _a4_halt',
  'SELECT 0 INTO @a4_assert');
PREPARE a4_halt_stmt FROM @a4_assert_sql;
EXECUTE a4_halt_stmt;
DEALLOCATE PREPARE a4_halt_stmt;
SELECT COUNT(*) INTO @a4_v_columns FROM information_schema.tables
WHERE table_schema='fulfilliq' AND engine='InnoDB'
  AND table_name IN ('raw_orders','raw_order_items','raw_sellers');
SET @a4_halt_cond = (@a4_v_columns <> 3);
SET @a4_assert_sql = IF(COALESCE(@a4_halt_cond,0) = 1,
  'SELECT `SQL_A_halted:_all_three_frozen_source_tables_must_be_InnoDB` FROM (SELECT 1) AS _a4_halt',
  'SELECT 0 INTO @a4_assert');
PREPARE a4_halt_stmt FROM @a4_assert_sql;
EXECUTE a4_halt_stmt;
DEALLOCATE PREPARE a4_halt_stmt;
...
SET @a4_extracted_at = UTC_TIMESTAMP(6);

-- No silent deduplication of business-key collisions, including invalids.
SELECT COUNT(*) INTO @a4_v_bad FROM (
  SELECT CAST(order_id AS BINARY) AS k FROM raw_orders
  GROUP BY CAST(order_id AS BINARY) HAVING COUNT(*)>1
) x;
SET @a4_halt_cond = (@a4_v_bad > 0);
SET @a4_assert_sql = IF(COALESCE(@a4_halt_cond,0) = 1,
  'SELECT `SQL_A_halted:_duplicate_raw_orders_order_id` FROM (SELECT 1) AS _a4_halt',
  'SELECT 0 INTO @a4_assert');
PREPARE a4_halt_stmt FROM @a4_assert_sql;
EXECUTE a4_halt_stmt;
DEALLOCATE PREPARE a4_halt_stmt;
...
SET @a4_halt_cond = (@a4_v_bad > 0);
SET @a4_assert_sql = IF(COALESCE(@a4_halt_cond,0) = 1,
  'SELECT `SQL_A_halted:_duplicate_raw_sellers_seller_id` FROM (SELECT 1) AS _a4_halt',
  'SELECT 0 INTO @a4_assert');
PREPARE a4_halt_stmt FROM @a4_assert_sql;
EXECUTE a4_halt_stmt;
DEALLOCATE PREPARE a4_halt_stmt;
...
SET @a4_halt_cond = (@a4_v_bad > 0);
SET @a4_assert_sql = IF(COALESCE(@a4_halt_cond,0) = 1,
  'SELECT `SQL_A_halted:_duplicate_raw_order_items_business_key` FROM (SELECT 1) AS _a4_halt',
  'SELECT 0 INTO @a4_assert');
PREPARE a4_halt_stmt FROM @a4_assert_sql;
EXECUTE a4_halt_stmt;
DEALLOCATE PREPARE a4_halt_stmt;
...
SELECT i.*, o.*, s.*, c.*,
  ...
  (invalid_item_key_row_n+orphan_order_item_row_n+orphan_seller_item_row_n
   +invalid_order_key_row_n+invalid_seller_key_row_n+invalid_purchase_order_n>0
   OR NULLIF(TRIM(@a4_documented_hold_reason),'') IS NOT NULL) AS operational_release_hold_flag,
  CONCAT_WS('|',
    IF(invalid_item_key_row_n+orphan_order_item_row_n+orphan_seller_item_row_n
      +invalid_order_key_row_n+invalid_seller_key_row_n+invalid_purchase_order_n>0,
      'SOURCE_ANOMALY_EFFECT_NOT_BOUNDED',NULL),NULLIF(TRIM(@a4_documented_hold_reason),''))
...
CREATE TABLE a4_provisional AS
...
CREATE TABLE a4_twin AS
  SELECT * FROM a4_provisional WHERE clock='T';
CREATE TABLE a4_compare AS
SELECT d.*,
...
CREATE TABLE a4_final_q AS
  SELECT * FROM a4_compare WHERE membership_D='YES'
    AND n4_fire_D=0 AND n5_action_disagree_flag=0;
CREATE TABLE a4_final_q_peer AS SELECT * FROM a4_final_q;

-- Exact pairwise rank: 1 + number strictly ahead; no rounded score sort.
CREATE TABLE a4_final_ranks AS
...
SELECT f.*,
  ...
  @a4_snapshot_id AS snapshot_id, @a4_source_version AS source_version,
  @a4_outcome_observation_boundary AS outcome_observation_boundary,
  @a4_temporal_convention AS source_temporal_convention,
  @a4_extracted_at AS extraction_timestamp_utc,
...
SELECT COUNT(*) INTO @a4_v_bad FROM a4_judged
...
SET @a4_halt_cond = (@a4_v_bad>0);
SET @a4_assert_sql = IF(COALESCE(@a4_halt_cond,0) = 1,
  'SELECT `SQL_A_halted:_judged_table_invariant_failure` FROM (SELECT 1) AS _a4_halt',
  'SELECT 0 INTO @a4_assert');
PREPARE a4_halt_stmt FROM @a4_assert_sql;
EXECUTE a4_halt_stmt;
DEALLOCATE PREPARE a4_halt_stmt;
SELECT (COUNT(*)<>COALESCE(MAX(valid_seller_n),0))
  ...
  OR (COUNT(DISTINCT seller_key)<>COUNT(*)) INTO @a4_v_bad FROM a4_judged;
SET @a4_halt_cond = (@a4_v_bad>0);
SET @a4_assert_sql = IF(COALESCE(@a4_halt_cond,0) = 1,
  'SELECT `SQL_A_halted:_seller_universe_capacity_invariant_failure` FROM (SELECT 1) AS _a4_halt',
  'SELECT 0 INTO @a4_assert');
PREPARE a4_halt_stmt FROM @a4_assert_sql;
EXECUTE a4_halt_stmt;
DEALLOCATE PREPARE a4_halt_stmt;
...
-- Persist judged seller table (lasting, not TEMPORARY).
DROP TABLE IF EXISTS a4_judged_seller;
CREATE TABLE a4_judged_seller AS
SELECT * FROM a4_judged
ORDER BY (`rank` IS NULL), `rank`, seller_key;
ALTER TABLE a4_judged_seller
  COMMENT = 'SQL A Stage 3 v0.2.1 judged sellers; C=20 S=20 simulation; no featured columns';

-- Result 1: final judged seller table. All sellers; qualifiers first.
SELECT * FROM a4_judged_seller ORDER BY (`rank` IS NULL), `rank`, seller_key;
-- Result 2: source/cohort audit, including unique customer-order totals.
SELECT @a4_snapshot_id AS snapshot_id, @a4_source_version AS source_version,
  @a4_extracted_at AS extraction_timestamp_utc, a.* FROM a4_audit a;
